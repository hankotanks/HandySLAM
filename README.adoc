...
For loader-specific CLI arguments, provide `--help` after the loader. For example, `./HandySLAM stray --help`.

=== Calibration

NOTE: Unless visual-inertial SLAM is being performed (`--imu`), this section can be safely ignored. 
All dataloaders assume perfect alignment between the LiDAR and camera when no profile is specified.
