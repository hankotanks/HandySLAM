...
cd HandySLAM
# create a python environment for dependencies (optional)
python3 -m venv ./env
...
For improved localization and mapping, the scenes should be upscaled using PromptDA.
If upscaling is performed, the `[--upscaled]` flag must be passed to `HandySLAM`.

NOTE: If a Python environment was active when building the project,
it must also be active when running PromptDA

[source,sh]
----
#!/bin/bash
./HandySLAM stray /path/to/scene --upscale
----
...
to record RGBD scenes. A standard calibration for an iPhone 16 Pro is given in `/profiles`.
